    ...
    <>
      ...
      <aside
        className={`
          ...
          w-full sm:w-[520px] max-w-full
          bg-newsprint-100 border-l-4 border-broadred
          overflow-y-auto
      ...
      >
        {country && (
          <div className="flex flex-col h-full">
            {/* Header */}
            <div className="px-6 pt-5 pb-0 border-b border-newsprint-400 bg-newsprint-200">
              <div className="flex items-start justify-between gap-3 pb-3">
                    ...
                    <div className="flex flex-wrap gap-x-3 gap-y-0.5 mt-2">
                      {wikidataCtx.capital && (
                        <span className="text-xs text-ink-muted">
                          Capital: <span className="text-ink-light">{wikidataCtx.capital}</span>
                        </span>
                      )}
                      {wikidataCtx.population && (
                        <span className="text-xs text-ink-muted">
                          Pop: <span className="text-ink-light">{formatPopulation(wikidataCtx.population)}</span>
                        </span>
                      )}
                      {wikidataCtx.region && (
                ...
                <button
                  onClick={onClose}
                  className="shrink-0 text-ink-muted hover:text-ink transition-colors p-1 -mt-1 -mr-1"
              ...
              <div className="flex gap-0 -mb-px">
                {[
                  ...
                  <button
                    ...
                    className={`
                      px-5 py-2.5 text-xs font-bold tracking-wide uppercase border-b-2 transition-colors duration-150
                      ${activeTab === tab.key
                        ? 'border-broadred text-broadred'
                      ...
                      <div className="space-y-3 mt-2">
                        {[1,2,3].map(i => (
                          <div key={i} className="space-y-1.5">
                            ...
                            <div className="h-3 bg-newsprint-300 rounded animate-pulse w-3/4" />
                            <div className="h-2 bg-newsprint-300 rounded animate-pulse w-1/3 mt-1" />
                          </div>
                        ))}
                      </div>
                    )}
